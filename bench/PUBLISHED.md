...
- **leap-c defer bucket disclosure.** Defer counts (excluded from excl-SKIP denominator): rust 76, python 58, go 45, zig 298, **c 17,549**. leap-c's defer is 0.43% of its excl-SKIP total — same order of magnitude as the 0.44pp gap to mainline's 99.9997%. A reviewer treating the deferred records as failures rather than excluded would see leap-c at ~99.13%, not 99.56%. The other four targets are all under 0.01% defer so the rebucketing is negligible for them. The cluster is the planner-perf hang in `random/index/*` and `random/groupby/*` (88 crashes + the timeouts that bucket as defer).
