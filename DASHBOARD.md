...
6. **Lane 4 still doesn't move with Phase 5 alone.** Harness opens in-memory DB; storage is slurp-on-open / commit-once-on-close. **Phase 4b per-commit WAL append is the actual lane-4 unlock** — scoped out this session with `SCOPE.md` documenting three structural blockers (harness DB-mode, Phase 4a multi-frame reader is stub, no page-dirty tracking). 17-22h follow-up session.
...
FTS5, R-tree, JSON1 beyond minimal, session/changeset, encryption, shell ornaments. Previously this list included Phase 5 async I/O — now landed (both backends on both targets). Phase 4b per-commit WAL append remains deferred (17-22h dedicated session; see `SCOPE.md`).
