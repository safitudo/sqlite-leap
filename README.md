...
2. **sqllogictest parity on the full 622-file upstream corpus, Linux x86_64 native.** All 5 leap targets pass **99.56–99.98% excl-SKIP** on the full corpus (~7M records, 70 min wall clock). Mainline SQLite runs the same corpus at **99.9997%** excl-SKIP. The largest gap is leap-c at 0.44pp behind mainline. Source: `tests/sqllogictest/results/corpus_2026_04_28_full/summary.json`. Per-target detail and crash counts in [`bench/PUBLISHED.md` §A.1](bench/PUBLISHED.md). Crashes are not hidden: leap-c 88 (planner-perf cluster), leap-python 17, leap-zig 11; leap-rust and leap-go 0; mainline 1.
