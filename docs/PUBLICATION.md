...
- **L4 — asymmetry closed 2026-04-27 evening (commit 677ff68); fresh Linux number pending.** mem-store v7-tx (`parts/storage/parts/mem-store/master.md` pin 17) implements BEGIN/COMMIT/ROLLBACK as snapshot-frame transactions. `lib_bench.c` and `lib_bench.rs` now route the SQL keywords to the new API. Local Mac arm64 re-run with the symmetry restored shows leap-c at 0.84× of mainline — i.e. leap-c LOSES on Mac once both sides do real transaction work. The publishable Linux number is still pending re-measurement and may be a smaller win, parity, or a loss. The 1.81× originally reported was the asymmetry, not the engine.
- **L3 PK-detection methodology gap closed (2026-04-27 evening, commit 770ceda).** Earlier drafts of this post flagged the L3 win as living in hand-written `src-c/examples/lib_bench.c` code. That code has now been deleted in favor of a spec-emitted helper (`pk_from_create_stmt`, pin 20). L3 still depends on the storage `IndexSpec::Rowid` path being efficient, but that path is also spec-described.
