...
>
> Not "does it beat SQLite" — it doesn't, on most measures, and where it does the comparison usually has a caveat. The interesting question is *how far* a single neutral spec carries you across five languages. This repo says: far enough that five hand-emitted engines agree byte-for-byte with mainline at fixed `.db` fixtures, ship as 361 KB (C, Mac) / 500 KB (C, Linux) to 4.6 MB (Go) binaries, and pass **99.56–99.98% of the records they attempt** on the full 622-file upstream sqllogictest corpus on Linux native — but **the denominator differs by target**: leap-python attempts 96.6% of mainline's records, leap-rust 89.5%, leap-zig 76.5%, leap-go 74.0%, **leap-c only 63.7%**. So "leap-c at 99.56%" means "leap-c attempts 63.7% of what mainline runs and passes 99.56% of that subset" — *not* "0.44pp behind mainline on the same corpus." Mainline runs the full corpus at 99.9997%. See `bench/PUBLISHED.md §A.1` for per-target denominators.
