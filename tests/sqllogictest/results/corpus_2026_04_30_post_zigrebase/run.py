...
OUT = f"{ROOT}/tests/sqllogictest/results/corpus_2026_04_29_post_zigrebase"
PER_FILE_TIMEOUT = 90  # default; per-target overrides below
PER_TARGET_TIMEOUT = {
    "python": 240,  # pure-Python interpreter ~100x slower; still passes given budget
    "sqlite": 60,
}
def timeout_for(target):
    return PER_TARGET_TIMEOUT.get(target, PER_FILE_TIMEOUT)
...
def run_one(args):
    ...
    t0 = time.time()
    try:
        cp = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout_for(target))
