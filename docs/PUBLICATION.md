...
- Run the full 622-file upstream `sqllogictest` corpus on Linux x86_64 native and pass 98.88–99.98% of what they attempt (post-2026-04-30, post-zigrebase). The 4 compiled targets (rust/c/zig/go) all attempt **99.93–99.96%** of mainline's record surface — denominator parity holds. leap-python attempts 98.94% (240s budget on the pure-Python interpreter still leaves 11 timeouts on heavy random/* files). leap-zig's 98.88% reflects 65,998 record-level FAILs the prior post-pcrebase build was masking via timeouts on the random/expr cluster — substantively the parity gap surfacing, not a regression. **Crashes: 0 across the 4 compiled leap targets.** leap-python has 3 (regressions from the longer 240s budget; under triage); mainline 0. (`PUBLISHED.md` §A.1)
...
2. **Full-corpus parity now holds at the denominator level for the 4 compiled leap targets.** The full 622-file v2 corpus on Linux x86_64 native (2026-04-30, post-zigrebase) gave rust/c/zig/go all at **99.93–99.96% exec/mainline** (within 0.07pp of mainline's record surface); excl-SKIP rates 98.88–99.98%. leap-python at 98.94%/99.98%. The 2026-04-28 baseline (when leap-c was at 63.7% exec/mainline and leap-zig at 76.5%) is archived in `PUBLISHED.md §A.1.archive`; the 36-pp denominator gap closed via Pin α29-pc-rebase (extended to all splice sites in 2026-04-30 commit f3493f3), Pin α23-c-bufslot-mat, and Pin C-mem-1/2.
...
Source: `tests/sqllogictest/results/corpus_2026_04_30_post_zigrebase/summary.json`. 622 upstream `.test` files × 6 targets, 90s per-file timeout (240s for Python, 60s for sqlite), 88 min wall on Ubuntu 22.04 (rustc 1.89, gcc 11.4, zig 0.16, go 1.25). Record-level pass-rate.

| Target | pass | fail | crashes | timeouts | total | exec/mainline | incl-SKIP | excl-SKIP |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| rust | 5,935,956 | 1,378 | 0 | 2 | 7,418,232 | **99.96%** | 80.02% | **99.98%** |
| c | 5,935,363 | 1,764 | 0 | 2 | 7,418,030 | **99.96%** | 80.01% | **99.97%** |
| zig | 5,870,897 | 65,998 | 0 | 2 | 7,418,032 | **99.96%** | 79.14% | **98.88%** |
| go | 5,929,882 | 4,653 | 0 | 2 | 7,415,420 | **99.93%** | 79.97% | **99.92%** |
| python | 5,860,466 | 1,134 | 3 | 11 | 7,342,283 | **98.94%** | 79.82% | **99.98%** |
| **mainline sqlite** | **5,939,855** | **19** | **0** | **0** | **7,420,713** | 100% | **80.04%** | **100.00%** |

Four things to read here:

- **`exec/mainline` is the denominator-asymmetry column** and must be quoted alongside excl-SKIP. Each target's excl-SKIP rate is computed against its own (target-specific) total. The 4 compiled leap targets (rust/c/zig/go) all attempt 99.93–99.96% of mainline's record surface — denominator parity holds. leap-python attempts 98.94% (240s budget on the pure-Python interpreter still leaves 11 timeouts on heavy random/* files).
- **excl-SKIP is the cross-engine comparison number, but only with `exec/mainline`.** With denominator parity now established for the 4 compiled targets, the excl-SKIP rates are directly comparable: rust 99.98%, c 99.97%, go 99.92%, zig 98.88%. **leap-zig's 98.88% reflects 65,998 record-level FAILs that the prior post-pcrebase build was masking via timeouts on the random/expr cluster** — substantively this is the parity gap surfacing, not a regression. The other compiled targets each carry under 5k FAILs.
- **incl-SKIP is misleading without further context.** Don't compare incl-SKIP across engines without converting to excl-SKIP and reporting `exec/mainline` first.
- **Crashes are nearly gone.** Compiled leap targets all have **0 crashes** in this run. leap-python has 3 (regressions from the longer 240s budget exposing previously masked paths; under triage). Mainline has 0.

**Per-file timeout:** 90 seconds (240s for Python, 60s for mainline).

**The 2 timeouts on every leap target are the same 2 files** — `select4.test` and `select5.test`. Both are deeply nested 5-way unindexed equijoin stress files; mainline finishes them with a planner-level rewrite. These 10 cells (2 × 5 leap targets) are the entire remaining timeout debt and are tracked for a planner follow-up.

**Earlier corpus measurements** are kept in `PUBLISHED.md §A.1.archive` (2026-04-28 baseline showing the 36.3pp denominator gap for leap-c, the 119-timeout cluster for leap-zig, and the 88 crashes — all closed by the Pin α29-pc-rebase + Pin α23-c-bufslot-mat + Pin C-mem cross-target spec waves landed 2026-04-29 and -30). The 335-file sample (post-G3, 2026-04-27) is in §A.2 and the v1 file-level run (2026-04-23) is in §A.3.
...
- **leap-c crash cluster (88 crashes, 2026-04-28 baseline) is closed** — Pin C-mem-1/2 + cross-target spec waves landed 2026-04-29; post-zigrebase the 4 compiled leap targets all carry 0 crashes on the full corpus. The remaining residual is leap-python's 3 crashes (longer 240s budget exposing previously-masked paths).
