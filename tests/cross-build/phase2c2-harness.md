# Phase 2c-2 test harness — language-neutral spec

The Phase 2c-2 harness drives the tokenize → parse → compile → vdbe.run pipeline with the expanded Phase 2c-2 grammar (WHERE clause, logical operators) and error surface. Same structure as phase2c1-harness; invariants extended by one.

## Location

Generated into `src-{lang}/bin/phase2c2-test` (or the target language's nearest equivalent).

## Invocation

Accepts exactly one positional argument: the path to `phase2c2.json`.

```
<harness-binary> <path-to-phase2c2.json>
```
...
For each case in `phase2c2.json.cases`, the harness:
...
2. Iterates `case.program` in source order. For each `step`:
   ...
   4. Verify the Program's well-formedness per `spec/vdbe-opcodes.spec.md` (invariants 1–12; see below). A violation is reported as `FAIL <case.name> at step <idx>: malformed-program: invariant <n>`.
...
## Well-formedness invariants (Phase 2c-2 set)

The harness checks invariants 1–12 from `spec/vdbe-opcodes.spec.md`:
...
Identical to `phase2c1-harness.md` § "Equality rules":
...
```
SUMMARY phase=2c2 target=<c|rust|wasm> passed=<int> failed=<int> total=<int>
```
