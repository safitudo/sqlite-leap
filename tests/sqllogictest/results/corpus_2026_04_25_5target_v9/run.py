...
OUT = f"{ROOT}/tests/sqllogictest/results/corpus_2026_04_25_5target_v9"
PER_FILE_TIMEOUT = 60
...
def main():
    ...
    summary_lines = ["# 5-target corpus run 2026-04-25 (post runner-format upgrade)\n",
    ...
    for t in TARGETS:
        ...
        summary_lines.append(f"| {t} | {s['pass']} | {s['fail']} | {s['defer']} | {s['skip']} | {tot} | {rate:.2f}% |")
        json_out[t] = {"pass":s["pass"], "fail":s["fail"], "defer":s["defer"], "skip":s["skip"], "total":tot, "rate":rate,
