...
- **L4 still depends on harness asymmetry.** PRAGMA / BEGIN / COMMIT / ROLLBACK are no-ops on the leap side of `lib_bench.c:454`, tagged `leaplint: target-local lift`. Mainline's bench path runs the begin/commit machinery; leap-c skips it. The L4 1.81× win is partly real (prepared-statement cache, predicate pushdown) and partly an asymmetric measurement. Spec promotion of the transaction-machinery wiring is the next P0 roadmap item (`docs/POST-PUBLICATION-ROADMAP.md`).
